...
class CallTrackingCompleted
{
								...
								)
    {
        ...
        $this->fullUrl = (strlen($fullUrl)<254) ? $fullUrl : substr($fullUrl,0,254);
...
/*        echo "<br> $query"."<hr>";
    ...
    static function createSt($values)
    {
        ...
        $query = "INSERT INTO `callTrackingCompleted` ($columns) VALUES ";
    ...
    {
        $db = DB::getInstance();
        $db->query("DELETE FROM 'callTrackingCompleted' WHERE `id`={$this->id} LIMIT 1");
        return (bool) $db->affected_rows;
    }
    public static function findById($id)
    {
        $id = intval($id);
        $db = DB::getInstance();
        $res = $db->query("SELECT * FROM `callTrackingCompleted` WHERE `id`=$id LIMIT 1")->fetch_assoc();
        if (!$res){
    ...
    {
        $db = DB::getInstance();
        return $db->query("SELECT * FROM `callTrackingCompleted`")->fetch_all(MYSQLI_ASSOC);
